...
def load_experiment_results(experiment_results_path):
    ...
    if isdir(experiment_results_path):
        files = [experiment_results_path + '/' +
                 f for f in listdir(experiment_results_path)]
...
def sample(model, domain_tensor, batch_size, gpu=False, chunk_size=5000):
    ...
    if len(domain_tensor) < chunk_size:
        ...
        samples = chunk_sample(model, domain_tensor,
                               batch_size, gpu=False, chunk_size=chunk_size)
...
def argmax(sample_x_y, known_X, target='sample', duplicates=False, top_n=1):
    ...
    if duplicates != False:
        ...
        keep = complement(sample_x_y.drop(target, axis=1),
                          known_X, boolean_out=True)
...
def safe_concat(dataframes: list[pd.DataFrame], sort: bool = False):
    """
    Functionally similar to `pd.concat`, but handles empty DataFrames by not concatenating them.
    """
    # Filter out None values and empty DataFrames
    dataframes_list = [
        df for df in dataframes if df is not None and not df.empty]

    # Return blank dataframe if dataframes_list is empty
    if not dataframes_list:
        return pd.DataFrame()

    # If only one DataFrame, return a copy
    if len(dataframes_list) == 1:
        return dataframes_list[0].copy()

    # Concatenate non-empty DataFrames
    return pd.concat(dataframes_list, sort=sort)
