...
def encode_component(df_column, encoding, name=''):
    ...
    if encoding.lower() == 'ohe':
        ...
        if 'FAILED' in smiles.values:
            ...
            # Resolve issue using edbo bot
            edbo_bot.talk(
                'The following names could not be converted to SMILES strings:')
            ...
            def not_response():
                ...
                for i in failed:
                    name_i = names[i]
                    text = edbo_bot.get_response(
                        'SMILES string for ' + name_i + '?')
                    new_smiles[i] = text

                edbo_bot.talk(
                    'OK computing Mordred descriptors for ' + name + '...')
...
def expand_space(index, descriptor_dict):
    ...
    for col in index.columns.values:
        ...
        expanded = [submatrix[submatrix[:, 0] == e][0]
                    for e in index[col].values]
        df = pd.DataFrame(
            expanded, columns=descriptor_dict[col].data.columns.values)
...
def reaction_space(component_dict, encoding={}, descriptor_matrices={},
    ...
    for key in component_dict:
        ...
        if decorrelate:
            try:
                des.uncorrelated(
                    threshold=decorrelation_threshold, target=None)
            except:
                None
