...
from .pd_utils import to_torch, join_to_df, argmax, complement, sample, torch_to_numpy, safe_concat
...
class thompson_sampling:
    ...
    def run(self, model, obj):
        ...
        self.samples = sample(model, domain, self.batch_size,
                              gpu=obj.gpu, chunk_size=self.chunk_size)
        ...
        for i in range(len(self.samples)):
            ...
            arg_maxs = safe_concat([arg_maxs, arg_max_i], sort=False)
...
class Kriging_believer:
    ...
    def run(self, model, obj):
        ...
        for i in range(self.batch_size):
            ...
            proposed = safe_concat([proposed, proposed_i], sort=False)
    ...
    def run(self, model, obj):
        ...
        if self.batch_size > 1:
            ...
            for i in range(len(samples)):
                ...
                arg_max_i = argmax(sample_i, known_X,
                                   duplicates=self.duplicates)
                arg_maxs = safe_concat([arg_maxs, arg_max_i], sort=False)

            proposed = safe_concat(
...
class eps_greedy:
    ...
    def run(self, model, obj):
        ...
        # Make choice list
        choice_list = [0] * round(self.eps * 1000) + \
            [1] * round((1 - self.eps) * 1000)
        ...
        for i in range(self.batch_size):
            ...
            # Append
            selected = safe_concat([selected, selected_i], sort=False)
