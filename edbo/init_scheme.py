...
from .pd_utils import safe_concat
...
class Init:
    ...
    def plot_choices(self, obj, export_path=None):
        ...
        X = safe_concat(
            [obj.domain.drop(self.experiments.index.values, axis=0), self.experiments])
        domain = ['Domain' for i in range(
            len(obj.domain.drop(self.experiments.index.values, axis=0)))]
...
def PAM(obj, batch_size, distance='gower', visualize=True,
    ...
    kmedoids_instance = kmedoids(
        sample, initial_medoids, metric=metric, tolerance=0.0001, itermax=300)
    ...
    if visualize == True:
        ...
        if len(sample[0]) < 4:
            ...
            for i in range(len(clusters)):
                ...
                tsne_data = safe_concat([tsne_data, data])
            embedding_plot(
...
def k_means(obj, batch_size, visualize=True, seed=None, export_path=None,
    ...
    for i in range(best):
        ...
        closest_i, _ = pairwise_distances_argmin_min(
            clusterer.cluster_centers_[[i]], cluster_i)
        closest = safe_concat([closest, cluster_i.iloc[closest_i]], sort=False)
